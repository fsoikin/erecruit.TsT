...
declare module linqjs {
	...
	interface IEnumerable<T> {
		...
		traverseDepthFirst<TResult>(func: (element: T) => IEnumerable<T>): IEnumerable<T>;
		traverseDepthFirst<TResult>(func: (element: T) => IEnumerable<T>, resultSelector?: (element: T, nestLevel: number) => TResult): IEnumerable<TResult>;
		...
		join<TInner, TKey, TResult>( inner: IEnumerable<TInner>, outerKeySelector: ( outer: T ) => TKey, innerKeySelector: ( inner: TInner ) => TKey, resultSelector: ( outer: T, inner: TInner) => TResult, compareSelector?: (obj: TKey) => any): IEnumerable<TResult>;
		join<TInner, TKey, TResult>(inner: { length: number;[x: number]: TInner; }, outerKeySelector: (outer: T) => TKey, innerKeySelector: (inner: TInner) => TKey, resultSelector: (outer: T, inner: TInner) => TResult, compareSelector?: (obj: TKey) => any): IEnumerable<TResult>;
		...
		isEmpty(): boolean;
		concat<T>( sequence: T[] ): IEnumerable<T>;
		concat<T>(...sequences: IEnumerable<T>[]): IEnumerable<T>;
		...
		toObject<TValue>( keySelector: ( element: T ) => string, elementSelector?: ( element: T ) => TValue ): { [key: string]: TValue; };
		toObject<TValue>( keySelector: ( element: T ) => number, elementSelector?: ( element: T ) => TValue ): { [key: number]: TValue; };
		// :IDictionary<TKey, T>
