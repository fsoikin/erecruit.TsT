...
namespace erecruit.vs
{
	public static class Core
	{
		...
		public static void Translate( IEnumerable<File> inputFiles )
		{
			...
			files
				.ToObservable()
			.Select( f => new { SourceFiles = new[] {f.Path}, OutputFile = f.Path } )
		...
		public static void WriteToOutputWindow( string msg )
		{
			...
			if ( ErrorHandler.Failed( output.GetPane( ref paneID, out var pane ) ) || pane == null ) return;
		...
		static bool HasConfigFile( string filePath )
		{
			var path = Path.GetDirectoryName( filePath );
			while ( !string.IsNullOrEmpty( path ) ) {
				if ( System.IO.File.Exists( Path.Combine( path, DefaultConfigFileName ) ) ) return true;
		...
		{
			var anyFilesWithoutConfig = inputFiles.Select( f => f.Path ).Select( HasConfigFile ).Contains( false );
			...
			inputFiles
				...
				.Select( proj => {
					...
					var configPath = Path.Combine( targetFolder.path, DefaultConfigFileName );
		...
		public static bool GetTranslateFlag( ProjectItem item )
		{
			...
			ctx.PropertyStorage.GetItemAttribute( ctx.ItemID, TranslateFlagName, out var result );
		...
		static FlagContext PrepareFlagOperation( ProjectItem item )
		{
			...
			hierarchy.ParseCanonicalName( firstFile, out var itemID );
