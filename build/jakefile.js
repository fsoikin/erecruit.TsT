...
desc("Clean");
task('clean', [], function () { return outputs.concat(lib).forEach(function (f) { return fs.existsSync(f) && fs.unlink(f); }); });
...
function compileTs(outFile, sources, prefixes, disableTypings, mergeOutput, prereqs) {
    if (mergeOutput === void 0) { mergeOutput = true; }
...
function prepend(prefixFiles, destinationFile) {
    ...
    prefixFiles
        ...
        .forEach(function (f) { return fs.appendFileSync(destinationFile, fs.readFileSync(f, enc)); });
...
function setVersion(versionValue) {
    ...
    function replace(file, regex) {
        ...
        var newContents = contents.replace(regex, function (_, prefix, __, suffix) { return prefix + versionValue + suffix; });
...
function updateLkg() {
    ...
    jake.rmRf(lkg);
    jake.mkdirP(lkg);
    outputs.forEach(function (f) { return jake.cpR(f, lkg); });
    ...
    function copyIfThere(where, copyTo) {
        ...
        if (fs.existsSync(from)) {
            ...
            list.include(["*.dll", "*.exe", "*.vsixmanifest", "*.msi"].map(function (f) { return path.resolve(from, f); }));
            list.exclude(["*vshost*"].map(function (f) { return path.resolve(from, f); }));
            list.toArray().forEach(function (f) { return jake.cpR(path.relative('.', f), to); });
        }
        else {
